...
void insertionSort(vector<int>& v) {
    ...
    for (int i = 1; i < n; i++) {
        int j;
        int key = v[i];
        for (j = i - 1; j >= 0 && v[j] > key; j--) { // shift elements greater than key to the right
            v[j + 1] = v[j];
        }
        v[j + 1] = key; // place key at its correct position
    }
}
...
// Custom comparator for sorting integers
bool myComparator(int& a, int& b) {
    // return a < b;  // increasing order sorting
    // return a > b;  // decreasing order sorting
}

// Custom comparator for sorting 2D vector based on the 1st index
